...
class Train:
    ...
    def loop(self, idx, ce, iteration, attack_num, debug=0):
        r = self._r
        w_num = len(self.w_list)
        num = 0
        num_pre = 0
        ...
        while num<iteration:
            ...
            ce_alt = r.evaluate(0)
            if ce_alt<=ce: # keep
                print("[%d, %d/%d] %d : " % (idx, num, iteration, attack_num), ce, ">", ce_alt)
                ce = ce_alt
                ret = 1
                hit = hit + 1
            else: # undo
                print("[%d, %d/%d] %d : " % (idx, num, iteration, attack_num), ce)
                for ws in attack_list:
                    widx = ws[0]
                    wi = ws[1]
                    w = self.w_list[widx]
            ...
            if num>0 and num % 100 == 0:
                print("hit rate:", hit - hit_pre, "/ 100 = ", float((hit - hit_pre)/(100)))
                print("hit rate:", hit, "/", num, "=", float(hit/num))
                num_pre = num
                hit_pre = hit
                r.save()
            #
            num += 1
        #
        print("hit rate:", hit, "/", iteration, "=", float(hit/iteration))
        r.save()
        return ce
