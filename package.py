...
#            self._w_save_path = self._w_float_path
        ...
        if self._package_id==0: # MNIST
            ...
            if config==0:
                ...
                # 3 : CNN : 16 x 16 x 8 > 16 x 16 x 16
                c = r.count_layers()
                cnn_2 = core.Conv_4_Layer(c, 16, 16, 8, 16, max_1, my_gpu)
                r.layers.append(cnn_2)
                # 4 : max : 16 x 16 x 16 > 8 x 8 x 16
                c = r.count_layers()
                max_2 = core.MaxLayer(c, 16, 16, 16, cnn_2, my_gpu)
                r.layers.append(max_2)

                # 5 : CNN : 8 x 8 x 16 > 8 x 8 x 32
                c = r.count_layers()
                cnn_3 = core.Conv_4_Layer(c, 8, 8, 16, 32, max_2, my_gpu)
                r.layers.append(cnn_3)
                # 6 : max : 8 x 8 x 32 > 4 x 4 x 32
                c = r.count_layers()
                max_3 = core.MaxLayer(c, 32, 8, 8, cnn_3, my_gpu)
                r.layers.append(max_3)

                # 7 : hidden : (4 x 4 x 32) x 64 = 512 x 64
                c = r.count_layers()
                hidden_1 = core.HiddenLayer(c, 512, 64, max_3, my_gpu)
                r.layers.append(hidden_1)
                # 8 : hidden : 64x64
                c = r.count_layers()
                hidden_2 = core.HiddenLayer(c, 64, 64, hidden_1, my_gpu)
